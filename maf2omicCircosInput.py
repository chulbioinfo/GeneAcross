...
def maf2linkmatrix(fNAME_Maf):
    sID_list = tre2SpeciesList(fNAME_tre)
    for i in range(len(sID_list)-1):
        sID = sID_list[i]
        sID2 = sID_list[i+1]
        tmp_list = sID_list
        fNAME_LinkMatrix = "omicCircos/work/link.matrix.tsv"
...
def maf2linkpg(fNAME_Maf,fNAME_tre,UpDownStreamSize):
    maf2linkmatrix(fNAME_Maf)
    sID_list = tre2SpeciesList(fNAME_tre)
...
def main():
    ...
    maf2linkpg(fNAME_Maf,fNAME_tre,UpDownStreamSize)
